...
const BusinessInsuCalculator: React.FC = () => {
  ...
  const handleCalculate = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      setSubmittedData({ industry, employees, coverage });
      setShowResults(true);
          ...
          <div className="container mb-8">
             <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-slate-200 pb-6">
                <div>
                   ...
                   <p className="text-slate-500 font-medium text-sm mt-2">Showing tailored matches for your <strong className="text-slate-700">{submittedData?.industry}</strong> enterprise.</p>
                </div>
             </div>
          </div>
