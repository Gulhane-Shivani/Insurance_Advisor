...
const CoverageOptions = ['50 Lac', '1 Cr', '2 Cr'];
...
const LifeInsuCalculator: React.FC = () => {
  ...
  const metricsData: Record<string, any> = {
    'plan-5': { ClaimSettlement: '99.2%', ClaimTime: '48 Hrs', discount: 'Online Discount' }, // HDFC
    'plan-11': { ClaimSettlement: '98.6%', ClaimTime: '24 Hrs', discount: '' }, // ICICI
    'plan-12': { ClaimSettlement: '99.5%', ClaimTime: '24 Hrs', discount: 'Inc. 5% off' }, // Tata AIG
  };

  const lifePlans = useMemo(() => insurancePlans.filter(p => p.type === 'life'), []);
  ...
  const handleCalculate = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setIsLoading(false);
      setSubmittedData({ covered, ageYou, smoker, coverage });
      setShowResults(true);
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="max-w-6xl mx-auto w-full text-center mb-2">
        <h2 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight">
          Calculate Your <span className="text-blue-600">Life Insurance</span> Premium
        </h2>
        <p className="text-slate-500 font-medium text-sm mt-3">Enter your details below to find the best life insurance plans tailored for you.</p>
      </div>
          ...
          <div className="container mb-8">
            <div className="flex flex-col md:flex-row md:items-end justify-between gap-4 border-b border-slate-200 pb-6">
              <div>
                ...
                <p className="text-slate-500 font-medium text-sm mt-2">Showing tailored matches for <strong className="text-slate-700">{submittedData?.covered}</strong> based on your profile.</p>
              </div>
            </div>
          </div>
...
export default LifeInsuCalculator;
