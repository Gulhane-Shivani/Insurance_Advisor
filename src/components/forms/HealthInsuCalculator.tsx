...
import { Check, X, Shield, Phone, Zap, Star, ChevronRight, Activity } from 'lucide-react';
    ...
    <div className="flex flex-col gap-8">
      ...
      <div className="max-w-6xl mx-auto w-full text-center mb-2">
        <h2 className="text-2xl md:text-3xl font-black text-slate-900 tracking-tight">
          Calculate Your <span className="text-green-600">Health Insurance</span> Premium
        </h2>
        <p className="text-slate-500 font-medium text-sm mt-3">Enter your details below to find the best health insurance plans tailored for you.</p>
      </div>
      ...
      <div className="bg-white rounded-[24px] p-4 md:p-5 shadow-xl shadow-slate-200/50 border border-slate-100 max-w-6xl mx-auto w-full relative overflow-hidden group">
        <div className="flex flex-col lg:flex-row lg:items-center gap-4 relative z-10 w-full">
          ...
          <form onSubmit={handleCalculate} className="flex-1 w-full flex flex-col sm:flex-row gap-3">
            <select 
              className="flex-1 min-w-0 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-green-500/20 focus:border-green-500 outline-none transition-all text-slate-700 font-bold text-sm"
              value={covered}
              onChange={e => setCovered(e.target.value as CoveredMember)}
              required
            >
              <option value="" disabled>Cover For</option>
              {['You', 'Couple', 'Family', 'Father', 'Mother'].map(opt => <option key={opt} value={opt}>{opt}</option>)}
            </select>
            
            <input 
              type="number" 
              placeholder="Your Age"
              className="flex-1 min-w-0 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:ring-2 focus:ring-green-500/20 focus:border-green-500 outline-none transition-all text-slate-900 font-bold text-sm" 
              value={ageYou}
            ...
            <select 
              ...
              required
            >
              <option value="" disabled>Coverage Area</option>
