...
const SectionTitle: React.FC<SectionTitleProps> = ({
  ...
  return (
    <div className={`mb-12 ${align === 'center' ? 'text-center mx-auto max-w-2xl' : ''}`}>
      ...
      <h2 className={`text-2xl md:text-4xl font-bold mb-6 leading-[1.15] ${isLight ? 'text-white' : 'text-slate-900'
        }`}>
        {title}
      </h2>
      {subtitle && (
        <p className={`text-base md:text-lg leading-relaxed ${isLight ? 'text-white/70' : 'text-slate-600'
          }`}>
          {subtitle}
        </p>
      )}
