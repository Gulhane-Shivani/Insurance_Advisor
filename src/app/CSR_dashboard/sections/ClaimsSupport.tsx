...
import { ShieldAlert, Clock, CheckCircle2, Search, Filter, FileText, Upload, AlertCircle, ChevronRight } from 'lucide-react';
...
const ClaimsSupport: React.FC = () => {
  ...
  const claims = [
    { id: 'CLM-2026-881', customer: 'Rajesh Kumar', type: 'Health', amount: '₹1,25,000', date: '28 Apr 2026', status: 'Pending Info', priority: 'High', facility: 'Apollo Mumbai' },
    { id: 'CLM-2026-882', customer: 'Anjali Sharma', type: 'Motor', amount: '₹45,000', date: '26 Apr 2026', status: 'Survey Done', priority: 'Medium', facility: 'Maruti Authorized' },
    { id: 'CLM-2026-883', customer: 'Sunil Gupta', type: 'Health', amount: '₹80,000', date: '20 Apr 2026', status: 'Approved', priority: 'Low', facility: 'Fortis Delhi' },
  ];
          ...
          <Card className="p-6 border-none shadow-xl shadow-slate-200/40">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex gap-2">
                <Button variant="outline" size="sm" icon={<Search size={14} />} onClick={() => toast.success('Opening claim search...')}>Search</Button>
                <Button variant="outline" size="sm" icon={<Filter size={14} />} onClick={() => toast.success('Opening filters...')}>Filter</Button>
              </div>
            </div>
            <div className="flex gap-2 mb-6 bg-slate-50 p-1 rounded-xl w-fit">
              {['All', 'Pending', 'In Progress', 'Resolved'].map(tab => (
                <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 py-2 rounded-lg text-[10px] font-black uppercase tracking-widest transition-all ${activeTab === tab ? 'bg-white text-violet-600 shadow-sm' : 'text-slate-400 hover:text-slate-800'}`}>{tab}</button>
              ))}
            </div>
                  ...
                  <div className="flex gap-4">
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <span className={`px-2 py-0.5 rounded text-[8px] font-black uppercase tracking-widest ${claim.status === 'Approved' ? 'bg-emerald-100 text-emerald-700' : 'bg-amber-100 text-amber-700'}`}>{claim.status}</span>
                      </div>
            ...
            <div className="space-y-6">
              ...
              <div className="p-4 bg-amber-500/10 border border-amber-500/20 rounded-2xl flex gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] font-medium text-slate-300 leading-relaxed">Discharge summary missing from Apollo Hospitals.</p>
                </div>
              </div>
              <div className="space-y-3">
                <h5 className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Support Tools</h5>
                <button onClick={() => toast.success('Opening file upload dialog...')} className="w-full text-left px-4 py-3 bg-white/5 hover:bg-white/10 rounded-xl text-xs font-bold transition-colors flex items-center gap-2">
                  <Upload size={14} className="text-violet-400" /> Upload Missing Documents
                </button>
                <button onClick={() => toast.success('Status update SMS sent to customer')} className="w-full text-left px-4 py-3 bg-white/5 hover:bg-white/10 rounded-xl text-xs font-bold transition-colors flex items-center gap-2">
                  <Clock size={14} className="text-violet-400" /> Send Status Update SMS
                </button>
                <button onClick={() => toast.success('Claim escalated to TPA Desk')} className="w-full text-left px-4 py-3 bg-white/5 hover:bg-white/10 rounded-xl text-xs font-bold transition-colors flex items-center gap-2">
