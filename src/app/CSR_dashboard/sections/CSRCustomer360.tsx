/* src/app/csr_dashboard/sections/CSRCustomer360.tsx */
import React, { useState } from 'react';
import { 
  Search, User, Phone, Mail, MapPin, ShieldCheck, 
      ...
      <Card className="p-8 border-none shadow-xl shadow-slate-200/40">
        ...
        <form onSubmit={handleSearch} className="flex flex-col md:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input 
              type="text" 
              placeholder="Search by Name, Phone, Email, or Policy Number..." 
              value={searchQuery}
          ...
          <div className="lg:col-span-4 space-y-6">
              ...
              <div className="px-6 pb-6 relative">
                ...
                <div className="mt-6 space-y-4">
                  <div className="flex items-center gap-3 text-sm font-bold text-slate-600">
                    <Phone size={16} className="text-slate-400" /> {customerData.phone}
                  </div>
                  <div className="flex items-center gap-3 text-sm font-bold text-slate-600">
                    <Mail size={16} className="text-slate-400" /> {customerData.email}
                  </div>
                  <div className="flex items-center gap-3 text-sm font-bold text-slate-600">
                    <MapPin size={16} className="text-slate-400" /> {customerData.address}
                  </div>
                </div>
              </div>
            </Card>
            ...
            <Card className="p-6 border-none shadow-xl shadow-slate-200/40">
              ...
              <div className="space-y-2">
                <button className="w-full text-left px-4 py-3 bg-slate-50 hover:bg-violet-50 hover:text-violet-700 rounded-xl text-xs font-bold transition-colors">Update Contact Info</button>
                <button className="w-full text-left px-4 py-3 bg-slate-50 hover:bg-violet-50 hover:text-violet-700 rounded-xl text-xs font-bold transition-colors">Send Payment Link</button>
                <button className="w-full text-left px-4 py-3 bg-slate-50 hover:bg-violet-50 hover:text-violet-700 rounded-xl text-xs font-bold transition-colors">Log New Ticket</button>
              </div>
            </Card>
          </div>
                ...
                {customerData.policies.map((policy) => (
                  <div key={policy.id} className="p-5 border border-slate-100 rounded-2xl flex flex-col md:flex-row md:items-center justify-between gap-4 hover:border-violet-200 transition-colors bg-slate-50">
                    ...
                    <div className="flex gap-2">
                      <Button variant="outline" size="sm" icon={<FileText size={14} />}>Details</Button>
                      <Button variant="outline" size="sm" icon={<Download size={14} />}>Doc</Button>
                    </div>
                  </div>
                ))}
              </div>
