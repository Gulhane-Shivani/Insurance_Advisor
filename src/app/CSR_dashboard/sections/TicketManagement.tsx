...
import { Ticket, Search, MessageSquare, AlertCircle, CheckCircle2, ChevronRight, User, Plus, X } from 'lucide-react';
...
const TicketManagement: React.FC = () => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.customer || !form.description) { toast.error('Fill in all required fields'); return; }
  ...
  const handleMarkResolved = () => {
    ...
    toast.success(`${selectedTicket.id} marked as resolved`);
  };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-lg animate-fade-in">
            <div className="p-6 border-b border-slate-100 flex items-center justify-between">
              ...
              <button onClick={() => setShowModal(false)} className="p-2 rounded-xl bg-slate-50 hover:bg-red-50 hover:text-red-500 transition-colors"><X size={20} /></button>
            </div>
            <form onSubmit={handleSubmit} className="p-6 space-y-5">
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" onClick={() => setShowModal(false)} className="flex-1 py-3 border-2 border-slate-200 text-slate-500 font-bold rounded-xl hover:border-slate-300 transition-all text-sm">Cancel</button>
        ...
        <div className="lg:w-2/3 space-y-6">
          <Card className="p-6 border-none shadow-xl shadow-slate-200/40">
            <div className="flex items-center justify-between mb-6">
              ...
              <div className="flex gap-2">
                <Button variant="outline" size="sm" icon={<Search size={14} />} onClick={() => toast.success('Opening ticket search...')}>Search</Button>
                <Button variant="primary" size="sm" icon={<Plus size={14} />} className="bg-violet-600 hover:bg-violet-700" onClick={() => setShowModal(true)}>New Ticket</Button>
              </div>
            </div>
          ...
          <Card className="p-6 border-none shadow-xl shadow-slate-200/40 bg-slate-900 text-white">
            ...
            <div className="space-y-6">
              ...
              <div className="space-y-3">
                ...
                <button onClick={() => toast.success(`Reply drafted for ${selectedTicket.customer}`)} className="w-full text-left px-4 py-3 bg-white/5 hover:bg-white/10 rounded-xl text-xs font-bold transition-colors flex items-center gap-2">
                  <MessageSquare size={14} className="text-violet-400" /> Reply to Customer
                </button>
                <button onClick={() => toast.success(`${selectedTicket.id} escalated to TPA Desk`)} className="w-full text-left px-4 py-3 bg-white/5 hover:bg-white/10 rounded-xl text-xs font-bold transition-colors flex items-center gap-2">
