...
const CSROverview: React.FC = () => {
  ...
  const tasks = [
    { id: 1, type: 'Renewal', customer: 'Rajesh Kumar', priority: 'High', due: '2h' },
    { id: 2, type: 'Claim', customer: 'Anjali Sharma', priority: 'Medium', due: '4h' },
    { id: 3, type: 'Service', customer: 'Sunil Gupta', priority: 'Low', due: '1d' },
    { id: 4, type: 'Query', customer: 'Priya Sharma', priority: 'High', due: '1h' },
  ];
        ...
        <div className="lg:col-span-8">
          <Card className="p-0 border-none shadow-xl shadow-slate-200/40 overflow-hidden bg-white">
            <div className="p-6 border-b border-slate-50 flex items-center justify-between bg-slate-50/50">
              ...
              <span className="px-3 py-1 bg-violet-600 text-white text-[10px] font-black rounded-lg uppercase tracking-widest">4 Critical</span>
            </div>
                  ...
                  {tasks.map((task) => (
                    <tr key={task.id} className="hover:bg-slate-50/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-right">
                        <button
                          onClick={() => { toast.loading(`Opening case for ${task.customer}...`, { duration: 800 }); setTimeout(() => toast.success('Case loaded'), 800); }}
          ...
          <Card className="p-6 border-none shadow-xl shadow-slate-200/40 bg-slate-900 text-white relative overflow-hidden">
            ...
            <div className="space-y-4">
              {tools.map((tool, i) => (
                <button
                  key={i}
                  onClick={() => toast.success(`Opening ${tool.label}...`)}
