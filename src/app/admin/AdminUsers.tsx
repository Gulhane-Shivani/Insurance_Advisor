    ...
    <div className="bg-white rounded-[32px] shadow-sm border border-slate-100 overflow-hidden">
      <div className="p-8 border-b border-slate-100 flex justify-between items-center bg-white">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1 font-medium">Manage registered users.</p>
        </div>
        ...
        <table className="min-w-full divide-y divide-slate-100">
          <thead className="bg-slate-50">
            <tr>
              ...
              <th className="px-8 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Joined</th>
          ...
          <tbody className="bg-white divide-y divide-slate-100">
            {currentUsers.map((user) => (
              <tr key={user.id} className="hover:bg-slate-50 transition-colors">
                <td className="px-8 py-5 whitespace-nowrap text-sm font-bold text-slate-400">#{user.id}</td>
                ...
                <td className="px-8 py-5 whitespace-nowrap text-sm font-medium text-slate-500">{user.created_at}</td>
                <td className="px-8 py-5 whitespace-nowrap">
      ...
      {totalPages > 1 && (
        <div className="px-8 py-5 border-t border-slate-100 flex items-center justify-between bg-white rounded-b-[32px]">
          <div className="text-sm text-slate-500 font-medium">
            ...
            of <span className="font-bold text-slate-900">{users.length}</span> results
          </div>
