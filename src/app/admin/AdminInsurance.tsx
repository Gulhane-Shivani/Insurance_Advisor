  ...
  return (
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
      <div className="p-6 border-b border-slate-200 flex justify-between items-center bg-white">
        <div>
          <h2 className="text-xl font-bold text-slate-800">Insurance Applications</h2>
          <p className="text-sm text-slate-500 mt-1">Review and approve insurance requests.</p>
        </div>
      ...
      <div className="overflow-x-auto">
        <table className="min-w-full divide-y divide-slate-200">
          <thead className="bg-slate-50">
            <tr>
              <th className="px-6 py-4 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Policy ID</th>
          ...
          <tbody className="bg-white divide-y divide-slate-100">
            {currentInsurances.map((ins) => (
              <tr key={ins.id} className="hover:bg-slate-50 transition-colors">
                <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-slate-800">#{ins.id}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-bold text-slate-600">{ins.user_name}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-500">{ins.type}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-800 font-bold">{ins.amount}</td>
                <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-slate-500">{ins.applied_date}</td>
      ...
      {totalPages > 1 && (
        <div className="px-6 py-4 border-t border-slate-200 flex items-center justify-between bg-slate-50">
          <div className="text-sm text-slate-500 font-medium">
            ...
            of <span className="font-bold text-slate-800">{insurances.length}</span> results
          </div>
